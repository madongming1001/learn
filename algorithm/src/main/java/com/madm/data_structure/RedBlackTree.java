...
public class RedBlackTree {
    ...
    public TreeNode search(int data) {
        ...
        while (tmp != null) {
            ...
            else if (tmp.data > data) tmp = tmp.left;
            else tmp = tmp.right;
        }
        return null;
    }
    ...
    private void remove(TreeNode node) {
        ...
        if (parent == null) {
            //子情况1，被删除结点是红黑树的根结点：
            root = successNode;
            if (successNode != null) successNode.parent = null;
        } else {
            if (successNode != null) successNode.parent = parent;
            if (parent.left == node) parent.left = successNode;
            else {
