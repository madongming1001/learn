 ...
 * 实际上目前还没有发现更好的在 大数据场景 中 准确计算 基数的高效算法，因此在不追求绝对精确的情
 * 况下，使用概率算法算是一个不错的解决方案.
 * 概率算法 不直接存储 数据集合本身，通过一定的 概率统计方法预估基数值，这种方法可以大大节省内
 * 存，同时保证误差控制在一定范围内。
 * Linear Counting(LC)：早期的基数估计算法，LC 在空间复杂度方面并不算优秀，实际上 LC 的空
 * 间复杂度与上文中简单 bitmap 方法是一样的（但是有个常数项级别的降低），都是 O(Nmax)
 * LogLog Counting(LLC)：LogLog Counting 相比于 LC 更加节省内存，空间复杂度只有
 * O(log2(log2(Nmax
 * )))
